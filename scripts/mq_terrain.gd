...
const heightmap_bind_index : int = 4
...
func _process(delta):
	if (waiting_for_compute && frame - last_compute_dispatch_frame >= num_waitframes_gpusync):
		...
		create_mesh()
	elif (!waiting_for_compute && !waiting_for_meshthread && false): # Disable for now
		run_compute()
...
func init_compute():
	...
	# Load compute shader
	var shader_file : RDShaderFile = load("res://compute/marching_cubes.glsl")
	...
	# Create lut buffer
	var lut = load_lut("res://compute/marching_cubes_lut.txt")
